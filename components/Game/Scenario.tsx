...
import { getCurrentInGameInventory, getCurrentScenario } from '../../reduxStore/slices/gameSlice'
import { Button, Text } from 'react-native-paper'
import { Option, RoleModifier, RoleResult, ScenarioOutcome } from '../../utils/types/option.interface'
...
const Scenario: React.FC<Props> = ({ scenario }) => {
  ...
  const runRoleEvent = (option: Option) => {
    ...
    setTimeout(() => {
      setOutcome(option.action(generateOutcomeTextBasedOnRole(roleResult, option)));
      setShowContinueBtn(true);
    }, 2000)
  }
  ...
  const generateOutcomeTextBasedOnRole = (_role: RoleResult, _selectedOption: Option): string => {
    if (!_role || !_selectedOption?.successNumber) return '';

    const isBaseCrit = _role.baseRole >= CRIT_SUCCESS || _role.baseRole <= CRIT_FAIL;
    let roleOutcomeText;

    if (isBaseCrit) roleOutcomeText = generateOutcomeText(_role?.baseRole, _selectedOption?.successNumber);
    else roleOutcomeText = generateOutcomeText(_role?.modifiedRole, _selectedOption?.successNumber);
  ...
  return (
    <View style={styles.main}>
      ...
      {showContinueBtn && <Button mode="contained" onPress={() => continueBtnPressed()}>Continue</Button>}
    </View>
  )
}
